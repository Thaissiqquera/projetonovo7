...
function renderClusterViz(clientes) {
  ...
  const x = d3.scaleLinear()
    .domain(d3.extent(clientes,d=>d.pca1)).nice()
    .range([40,c.clientWidth-20]);
  const y = d3.scaleLinear()
    .domain(d3.extent(clientes,d=>d.pca2)).nice()
    .range([c.clientHeight-20,20]);
  const color = d3.scaleOrdinal(d3.schemeCategory10);
  svg.selectAll('circle').data(clientes)
    .enter().append('circle')
      .attr('class','dot')
      .attr('cx',d=>x(d.pca1))
      .attr('cy',d=>y(d.pca2))
  ...
  svg.append('g')
    .attr('transform',`translate(0,${c.clientHeight-20})`)
...
function renderClusterInfo(clusters) {
  ...
  clusters.forEach(c=>{
    ...
    d.innerHTML=`<h4>Cluster ${c.id}: ${c.tipo}</h4>`+
      `<ul><li>Frequência média: ${c.frequencia_media.toFixed(2)}</li>`+
      `<li>Gasto médio: R$ ${c.gasto_total_medio.toFixed(2)}</li>`+
      `<li>Dias desde última compra (média): ${c.dias_ultima_compra.toFixed(2)}</li>`+
      `<li>Clientes: ${c.clientes_count}</li></ul>`;
    e.appendChild(d);
  });
}
...
function loadRegressao() {
  fetch('/api/regressao')
    .then(r=>r.json()).then(d=>{
      renderBar('coef-campanhas', d.regressao_campanhas.coeficientes.map(x=>x.variavel), d.regressao_campanhas.coeficientes.map(x=>x.coeficiente), 'Campanhas');
      renderBar('coef-clientes', d.regressao_clientes.coeficientes.map(x=>x.variavel), d.regressao_clientes.coeficientes.map(x=>x.coeficiente), 'Perfil Cliente');
    });
}
...
function showClvStats(s) {
  ...
  e.innerHTML = Object.entries(s).map(([k,v]) =>
    `<div class="clv-stat"><span>${k.replace(/_/g,' ')}</span><span>${typeof v==='number'? (v.toFixed? 'R$ '+v.toFixed(2):v) : v}</span></div>`
  ).join('');
}
